...
async def analyze_invoices(request: AnalyzeRequest):
    ...
    Args:
        request: AnalyzeRequest containing session_id and question
    
    Returns:
        AnalyzeResponse with answer, optional table data, and optional graph
    """
    # Get session data
    ...
    if session_data is None:
        raise HTTPException(
            status_code=404, 
            detail="Session not found or expired. Please upload files again."
        )
    ...
    try:
        ...
        # Extract results
        answer = result.get("answer", "No relevant information found in the invoices.")
        table_df = result.get("table")
        graph_fig = result.get("graph_fig")
        
        # Convert table to list of dictionaries if it exists
